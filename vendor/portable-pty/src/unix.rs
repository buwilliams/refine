...
/// Mark the currently inherited non-stdio descriptors close-on-exec before
/// `Command::spawn` creates its private child-error pipe.
///
/// Closing every descriptor from `pre_exec` also closes Rust's private pipe.
/// When `execve` then fails (for example with E2BIG), the standard library
/// cannot report that errno to the parent and the child aborts while writing
/// the error. Applying FD_CLOEXEC in the parent provides the same successful
/// exec hygiene without invalidating Rust's later-created error descriptor.
fn mark_random_fds_cloexec() {
    if let Ok(dir) = std::fs::read_dir("/dev/fd") {
        for fd in dir.filter_map(|entry| {
            entry
                .ok()
