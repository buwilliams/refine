...
**Changes since September 2, 2026, through 4.3.1.** The 4.3 series brings agent instructions into one place, makes Goal progress and failures easier to follow, improves workflow recovery, and introduces Hubs for documentation and reports, including the built-in Metrics Hub.
...
## Understand Refine usage with Metrics Hub

**Metrics Hub now ships with Refine.** Open it from **New Goal → Hub** or **Settings → Hubs** to understand how your team uses Refine across the target app's synced Goals.

Choose **Day**, **Week**, **Month**, **Quarter**, or **Year** to see Goals created and delivered, average rounds per Goal, and median delivery time. The views cover rolling windows of 24 hours, 7 days, 30 days, 90 days, and 365 days. Comparisons show the preceding window of the same length.

![Metrics Hub showing the Month view, key statistics, and delivery trends](images/4.3/metrics-overview.png)

*The Month view brings incoming work, delivery, and iteration together. Screenshots show a saved report from the Refine project; your numbers will reflect your target app.*

The round breakdown shows how often Goals take one, two, or several rounds. Current workload highlights unfinished and aging work, while node and reporter activity show where work is happening and who is creating Goals. Current workload includes all retained Goals, regardless of the selected time horizon.

![Metrics Hub showing rounds per Goal and current workload](images/4.3/metrics-rounds.png)

**Keep the report fresh.** Choose **Refresh statistics** to calculate and save a snapshot without launching an agent or changing Goals. The report shows when it was updated and flags snapshots older than 24 hours. Saved statistics synchronize through refine-state, and **Download JSON** lets you use them in another reporting tool.

The included **Update Metrics Hub** Skill refreshes the snapshot and checks the report. Run it manually, or attach it to events in **Settings → Prompts** for automatic maintenance. Automation is opt-in. The Hub and its Skill cannot be removed, but you can edit the Skill's instructions.

Delivery counts require a recorded final integration, and delivery time includes waiting. More rounds do not necessarily mean defects. Coverage notes explain missing dates so you can interpret the report with context. [Learn how each metric is calculated](../product/metrics.md).

## Build and maintain Hubs with Skills

Choose a reusable Skill when creating a Hub. Run it manually to build or refresh the Hub, or assign it to events for automatic updates. Hub details are included through editable prompt templates, and the association synchronizes with the target app’s state.

Refine Hub includes an editable **Update Refine Hub** Skill that cannot be removed. Run it with a description of the documentation or release notes you need. [Maintain a Hub](../authoring.md).
...
Use **Settings → Hubs** to create sites for a team handbook, reports, or other project knowledge. Upload website files and assets, manage collections of records, and preview a site before publishing it. The same capability is available to agents through the CLI, API, and MCP.
...
![Refine Hub and its maintenance Skill in Settings → Hubs](images/4.3/hubs.png)
...
The **New Goal** menu brings together creation actions, Hub sites, and workflow and target-application status. Buttons beneath each status expose its available actions. Developer feedback is labeled **Contact Refine Devs**.
