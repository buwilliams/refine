...
Plan Mode explores intent before a user persists work. The Goal workflow Plan step instead invokes Skills bound to its Enter Event against the current Round and repository. Each Plan Skill chooses its method and returns a finalized plan; the workflow collects independent plans before Implement. There is no mandatory proposal, criticism, revision, or pre-check agent sequence. A finding recovery Round invokes the Plan Skill with its scoped correction request and retained candidate. These plans guide execution without changing the Goal or replacing Quality, Governance, or Review.
