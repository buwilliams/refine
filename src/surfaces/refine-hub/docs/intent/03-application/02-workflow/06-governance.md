...
Provider errors, unreadable verdicts, supersession races, refresh conflicts, and integration failures retain their original candidate, branch, handoff, gate evidence, and target snapshots. No hidden provider-report repair or generated recovery Round replaces that history. Selecting Plan retains the existing authored Round. Before evaluation, the [shared preparation capability](11-consistency-contract.md) may reconstruct missing inputs or record recovery to a fresh execution branch; replacement candidate checks remain required.

Integration rereads the current step occurrence immediately before each target compare-and-swap and before starting a push. Fetching and merging in the private worktree do not permit later publication after a decision changes. An already started publication child finishes its individual Git operation; the next publication requires another current-occurrence check. The local integration result is recorded before push as incomplete history, so interruption or push failure preserves that fact without claiming completed integration. A replacement can finish the remaining publication when the original decision is still current.

An integration finishing after a later workflow decision records its actual Git result against the original request. It cannot replace that request's redirected outcome, advance the later step, or supply success to a different Governance occurrence. A decision cannot erase a push or merge that already happened.
...
The accepted integration decision pins its original Round, occurrence, node, branch, base, candidate, target, and remote atomically. If the request continuation is lost, the existing Goal scheduler resumes that pending operation through the same integration capability. Repeated requests reuse completed effects; changed inputs fail the original operation without integrating replacement work. A node transfer or redirect cannot prevent historical recording of an effect already produced, but it prevents that old completion from advancing current work. Legacy pending requests without verifiable original inputs are settled visibly rather than replayed against new inputs.
