...
Workflow step and trigger views show assigned Skills and prompt previews. Shared resources contains the Templates used across those steps; selecting a step does not create separate copies of them. The shared Template catalog includes an expandable composition map. Each entry explains where Refine uses it. Templates build prompts; partials are reusable pieces included by those templates. Solid lines follow saved references, and dashed lines show pieces selected by a launch when needed. Shared partials appear once. The All entries view also identifies reference entries without a current built-in launch.
