...
- A Goal never schedules on its owning node: read the Goal's own round logs
  ...
  state. See "Refine-owned durable state" in `src/surfaces/refine-hub/docs/runbooks/install.md` for
...
automatically (see `src/surfaces/refine-hub/docs/runbooks/state-sync-recovery.md`).
