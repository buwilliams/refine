...
Both menus identify their context with a heading. Main, Node, Reporter, and Tools open their menus beside the top of the row, moving upward when needed to fit the screen. Their full rows respond to clicks and keyboard focus.
...
Open **Main**, marked with a panels icon, to select a screen. Dashboard opens initially. Each open screen stays listed beneath Main when the menu closes. Selecting its row, menu item, or Search command returns to its saved filters, expanded panels, selection, drafts, and scroll position. Selecting the same screen again does not open a duplicate. Explicit links with different filters update that screen.

Use a screen’s **Close** button to release it. Unsaved Settings work requires confirmation. Closing the active screen selects the first remaining Main screen, or opens Dashboard if none remain. Content and drafts persist during this page session; a browser reload reconstructs the current URL and does not retain unsaved forms.

Changing Node or target app invalidates retained data. Local Node switching confirms unsaved work; an external context change keeps draft screens visible with a warning and disabled controls until discarded.

**Skills** and **Hubs** expand and collapse independently. Refine remembers these choices and whether the rail itself is collapsed.
...
Switching screens preserves running sessions. Returning to any open Main screen restores its retained content. Browser Back and Forward also move between screens and windows. A window’s close action is labeled **Close and stop** when it owns a running terminal or agent.
