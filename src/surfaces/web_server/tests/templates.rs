...
fn template_reset_api_restores_only_selected_templates() {
    ...
    fs::create_dir_all(&root).unwrap(); git(&root, &["init", "-q"]).unwrap();
    let mut server = server_with_projection(); server.target_root = Some(root.clone());
    let request = |method: &str,path: &str,body|server.handle(ApiRequest{method:method.into(),path:path.into(),body});
    for id in ["workflow","planning-agent"] {
        assert_eq!(request("PUT",&format!("/api/templates/{id}"),Some(json!({"revision":0,"prompt":"Custom"}))).status,200);
    }
    assert_eq!(request("POST","/api/templates/reset",Some(json!({"revisions":{"workflow":0}}))).status,409);
    assert_eq!(request("POST","/api/templates/reset",Some(json!({"revisions":{"workflow":1}}))).status,200);
    ...
    assert_eq!(request("GET","/api/templates/planning-agent",None).body["item"]["prompt"],"Custom");
    fs::remove_dir_all(root).unwrap();
}
