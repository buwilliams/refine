...
const GUIDE_CATEGORIES = [
  {
    ...
    items: [
      ...
      guideItem("quickstart-create-node", "Create node", "Configure",
        "Create an node so this machine owns its Gaps and local runtime settings.",
        "Action: create an node for this machine.",
        { hash: "#/node/application", selector: "#node-add" },
        { canUseDefault: false }),
      guideItem("quickstart-generate-ai", "Generate with AI", "Configure",
        "Let the AI generator draft the target-app start, stop, and build commands from the codebase.",
      ...
      guideItem("quickstart-start", "Start", "Configure",
        "Start the target app from Node process management to confirm the commands work.",
  ...
  },
  {
    ...
    items: [
      guideItem("node-educate", "Settings only for this machine", "Educate",
        "Node settings belong to this machine. Use them for local runtime, reporters, and target-app commands that should not apply to every refine node.",
        "Default: each machine keeps its own active node settings.",
        { hash: "#/node/application", selector: "#settings-tabs" }),
      guideItem("node-create", "Create an node", "Educate and configure",
        "Create an node when this machine, operator, or environment should own separate Gaps and local runtime settings.",
        "Action: create when setting up a new machine.",
        { hash: "#/node/application", selector: "#node-add" },
        { canUseDefault: false }),
      guideItem("node-active", "Activate node", "Educate and configure",
        "The active node controls local ownership and node-scoped settings. Switch it before changing reporters or application commands for another machine.",
      ...
      guideItem("node-manage", "Nodes", "Educate and configure",
        ...
        { hash: "#/node/application", selector: "#node-add" }),
      guideItem("cluster-manage", "Remote node operations", "Educate and configure",
        "Remote node fields on Nodes enable SSH bootstrap and maintenance for the same target app.",
        "Default: leave remote fields empty unless the project needs another Refine node.",
        { hash: "#/node/application", selector: "#node-remote-add" },
        { canUseDefault: false }),
      ...
      guideItem("application-ai", "Generate with AI button", "Educate and configure",
        "The AI generator analyses the codebase, writes a .refine/manage-app.sh wrapper (start, stop, build, status — with detailed logging), and points the target-app commands at it. Edit the script or the saved commands afterward if you have a more reliable app-specific approach.",
      ...
      guideItem("application-start", "Start", "Educate and configure",
        ...
        { hash: "#/node/target-app", selector: "#s-target-start-command" }),
      guideItem("application-stop", "Stop", "Educate and configure",
        "The stop command should be idempotent so refine can stop or build the target app without manual cleanup.",
        "Default: blank until you know the local stop command.",
        { hash: "#/node/target-app", selector: "#s-target-stop-command" }),
      guideItem("application-build", "Build", "Educate and configure",
        "The build command prepares generated artifacts after merged work. Use a specific refine build script when the app needs setup beyond the normal build command.",
        "Default: blank unless the app has a reliable build command.",
        { hash: "#/node/target-app", selector: "#s-target-build-command" }),
      guideItem("application-auto-build", "Automatic application build", "Educate and configure",
        "Automatic build controls when merged work is built before review.",
      ...
      guideItem("application-build-timeout", "Build timeout", "Educate and optional config",
        "Build timeout is how long Refine waits for the build command before leaving build work failed or incomplete.",
        "Default: 300 seconds.",
      ...
      guideItem("runtime-parallel-run-cap", "Parallel-run cap", "Educate and configure",
        "Parallel-run cap limits how many Gap agent runs this node can launch at once.",
        "Default: 5.",
      ...
      guideItem("runtime-backlog-promote", "Auto-promote backlog to todo", "Educate and configure",
        "Auto-promote backlog to todo controls how long the Workflow Engine leaves backlog Gaps alone before making them eligible for work.",
      ...
      guideItem("runtime-project-update-pulse", "Target repo update pulse", "Educate and configure",
        "Target repo update pulse checks for local or upstream commits and refreshes this node's projected state.",
        "Default: 1 minute.",
  ...
  },
  {
    ...
    items: [
      ...
      guideItem("quality-gate", "Quality Gate", "Configure",
        "Choose whether QA runs before merge in a Gap worktree or after the shared application build.",
      ...
      guideItem("application-test", "Target-app tests", "Educate and configure",
        "Target-app tests are the supervised command Refine runs during workflow QA.",
        "Default: configure the command that matches the target app test runner.",
        { hash: "#/node/target-app", selector: "#s-target-test-command" }),
  ...
  },
  {
    ...
    items: [
      ...
      guideItem("process-pause-workflow", "Pause or unpause workflow", "Educate",
        "Pausing workflow automation keeps the UI running while agents, queued builds, QA, merges, and active background operations wait.",
      ...
      guideItem("process-agent-processes", "Agents", "Educate",
        "Agents lists active calls to the configured agent provider, including Gap agents and chat sessions.",
      ...
      guideItem("process-runner-processes", "Subprocess history", "Educate",
        "Subprocess history shows background work such as rebuilds, merges, projection-cache work, and queued operations.",
  ...
  },
  {
    ...
    items: [
      guideItem("nav-application-status", "Application status", "Educate",
        "The application status indicator shows target-app state and opens the Node process view for start, stop, build, sync, and checks.",
      ...
      guideItem("nav-agent-status", "Agent status", "Educate",
        "The agent status pill summarizes active or paused agent work and links to Node processes.",
