...
function renderTodoPanel() {
    ...
    return `
      <section class="todo-panel" data-testid="toolbar-todo-panel">
        <div class="todo-panel-header">
          <div>
            <h3>Todo List</h3>
            <p class="muted small">Select a Reporter in Controls to open their saved todo lists.</p>
          </div>
        </div>
      </section>`;
  }
  ...
  return `
    <section class="todo-panel" data-testid="toolbar-todo-panel">
      <div class="todo-panel-header">
        <div>
          <h3>Todo List <span class="todo-reporter">${htmlEscape(reporter)}</span></h3>
          <p class="muted small" data-testid="todo-persistence-label">
            Saved for this Reporter in the target app and available from other nodes.
          </p>
        </div>
        <button type="button" class="secondary small" data-todo-refresh
                data-testid="todo-refresh" ${todoState.loading ? "disabled" : ""}>Refresh</button>
      </div>
      ${todoState.error ? `
        <div class="todo-message error" role="alert" data-testid="todo-error">${htmlEscape(todoState.error)}</div>
      ` : ""}
      ${todoState.loading && todoState.reporter === reporter ? `
        <div class="todo-message muted" data-testid="todo-loading">Loading todo lists…</div>
      ` : `
        <div class="todo-list-tools">
          <label>
            <span>List</span>
            <select data-todo-list-selector data-testid="todo-list-selector">
              ${todoState.lists.length
                ? todoState.lists.map((candidate) => `
                    <option value="${htmlEscape(candidate.id)}"
                            ${candidate.id === todoState.selectedListId ? "selected" : ""}>
                      ${htmlEscape(candidate.name)}
                    </option>`).join("")
                : `<option value="">No lists yet</option>`}
            </select>
          </label>
          <button type="button" class="secondary" data-todo-new-list data-testid="todo-new-list">New list</button>
        </div>
        ${list ? `
          <form class="todo-list-name-form" data-todo-list-name-form>
            <label>
              <span>List name</span>
              <input type="text" value="${htmlEscape(list.name)}" maxlength="120"
                     data-todo-list-name data-testid="todo-list-name" required>
            </label>
            <button type="submit" class="secondary" data-testid="todo-list-rename">Save name</button>
          ...
          <form class="todo-add-form" data-todo-add-form>
            ...
            <input id="todo-item-text" type="text" maxlength="4000"
                   placeholder="Add a todo…" data-todo-item-text data-testid="todo-item-text" required>
            <button type="submit" data-testid="todo-add-item">Add</button>
          </form>
          ${items.length ? `
            <ul class="todo-items" data-testid="todo-items">
              ${items.map((item) => `
                <li class="todo-item${item.done ? " done" : ""}" data-todo-item-id="${htmlEscape(item.id)}">
                  <button type="button" class="todo-done-toggle ${item.done ? "secondary" : ""}"
                          data-todo-toggle data-testid="todo-item-toggle"
                          aria-label="${item.done ? "Undo" : "Mark done"} ${htmlEscape(item.text)}">
                    ${item.done ? "Undo" : "Done"}
                  </button>
                  <span class="todo-item-text">${htmlEscape(item.text)}</span>
                  <span class="todo-item-actions">
                    <button type="button" class="secondary small" data-todo-edit
                            data-testid="todo-item-edit">Edit</button>
                    <button type="button" class="secondary small danger" data-todo-delete
                            data-testid="todo-item-delete">Delete</button>
                  </span>
                </li>`).join("")}
            </ul>
          ` : `
            <div class="todo-empty muted" data-testid="todo-empty">No todos in this list.</div>
          `}
        ` : `
          <div class="todo-empty muted" data-testid="todo-no-lists">
            Create a list to start adding todos for ${htmlEscape(reporter)}.
          </div>
        `}
      `}
    </section>`;
...
function bindTodoPanel(root) {
  bindOnce(root.querySelector("[data-todo-refresh]"), "click", () => {
    void loadTodoListsForReporter(state.lastReporter);
  });
  bindOnce(root.querySelector("[data-todo-list-selector]"), "change", (event) => {
    todoState.selectedListId = event.target.value || "";
    drawToolbar();
  });
  bindOnce(root.querySelector("[data-todo-new-list]"), "click", async () => {
  ...
  $$("[data-todo-item-id]", root).forEach((row) => {
    const item = list.items.find((candidate) => candidate.id === row.dataset.todoItemId);
    if (!item) return;
    ...
    bindOnce(row.querySelector("[data-todo-edit]"), "click", async () => {
      const text = await modalPrompt("Todo", item.text, { title: "Edit todo" });
