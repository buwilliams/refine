...
function renderTemplatesCatalog(data = {}) {
  ...
  return `<section class="settings-section" data-testid="settings-templates">
    ...
    <div data-resource-map-host>${renderResourceMap(data)}</div>
...
function bindTemplatesCatalog(data) {
  ...
  if (!section) return;
  bindResourceMap(data);
  section.querySelectorAll("[data-resource-reset]").forEach(button => { button.onclick = () => openResourceReset({id:button.dataset.resourceReset}); });
