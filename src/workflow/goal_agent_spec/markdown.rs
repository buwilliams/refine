...
pub(super) fn render_object_fields(
...
pub(super) fn push_scalar_field(output: &mut String, label: &str, value: &Value) {
    output.push_str(&format!("- **{label}:** {}\n", scalar_text(value)));
}

pub(super) fn push_prose(output: &mut String, heading: &str, prose: &str) {
...
pub(super) fn push_heading(output: &mut String, level: usize, label: &str) {
    if !output.is_empty() && !output.ends_with("\n\n") {
        output.push('\n');
    }
...
pub(super) fn fallback(output: String, text: &str) -> String {
...
pub(super) fn meaningful_string(value: Option<&Value>) -> Option<&str> {
    value
...
pub(super) fn indent_continuation(value: &str, indent: &str) -> String {
    value
        .trim()
        .lines()
        .enumerate()
