...
impl FileTargetAppService {
    ...
    pub fn write_manage_app_wrapper(
        ...
        config.rebuild_command = "./.refine/manage-app.sh rebuild".to_string();
...
fn manage_app_wrapper_script(config: &TargetAppGeneratedConfig) -> String {
    ...
    let mut lines = vec![
        ...
        format!(
            "REBUILD_COMMAND={}",
            shell_quote(config.rebuild_command.trim())
        ),
        format!(
            "STATUS_COMMAND={}",
    ...
    lines.extend([
        ...
        "  rebuild) run_cmd \"$REBUILD_COMMAND\" ;;".to_string(),
        "  status) run_cmd \"$STATUS_COMMAND\" ;;".to_string(),
        "  *)".to_string(),
        "    printf 'usage: %s start|stop|rebuild|status\\n' \"$0\" >&2".to_string(),
        "    exit 64".to_string(),
        "    ;;".to_string(),
        "esac".to_string(),
    ]);
...
mod tests {
    ...
    fn target_app_service_writes_manage_app_wrapper() {
        ...
        let mut config = TargetAppGeneratedConfig {
            ...
            rebuild_command: "printf rebuilt > ../rebuilt".to_string(),
