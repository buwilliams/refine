...
#[test]
fn file_work_item_service_bulk_status_cancels_active_and_review_goals_but_not_done() {
    let temp_root = unique_temp_dir("work-item-bulk-cancel");
    ...
    for id in ["ACTIVE", "QUALITY", "REVIEW", "FAILED", "DONE"] {
        service.create_goal_summary(id, Some(id)).unwrap();
    }
    service
        .set_goal_status_unchecked("ACTIVE", &GoalStatus::Todo)
        .unwrap();
    WorkflowEngine::with_target_root(&runtime_root, &target_root)
        .claim("ACTIVE")
        .unwrap();
    for (id, status) in [
        ("ACTIVE", GoalStatus::InProgress),
        ("QUALITY", GoalStatus::Qa),
        ("REVIEW", GoalStatus::Review),
        ("FAILED", GoalStatus::Failed),
        ("DONE", GoalStatus::Done),
    ] {
        service.set_goal_status_unchecked(id, &status).unwrap();
    }
    ...
    let result = service
        .bulk_update_goals(
            BulkGoalSelection {
                selected_ids: Some(
                    ["ACTIVE", "QUALITY", "REVIEW", "FAILED", "DONE"]
                        .into_iter()
                        .map(str::to_string)
                        .collect(),
                ),
            ...
            BulkGoalUpdate::Status("cancelled".to_string()),
        )
        .unwrap();

    assert_eq!(result.updated, 4);
    assert_eq!(result.ids, vec!["ACTIVE", "FAILED", "QUALITY", "REVIEW"]);
    assert_eq!(result.skipped, 1);
    assert_eq!(result.skipped_details[0].id, "DONE");
    assert_eq!(result.skipped_details[0].reason, "status:done");
    for id in ["ACTIVE", "QUALITY", "REVIEW", "FAILED"] {
        assert_eq!(
            service.show_goal_summary(id).unwrap().goal.status,
    ...
    assert_eq!(
        service.show_goal_summary("DONE").unwrap().goal.status,
        GoalStatus::Done
    );
...
#[test]
fn bulk_cancel_revalidates_done_status_after_selection() {
    let temp_root = unique_temp_dir("work-item-bulk-cancel-selection-race");
    let refine_dir = temp_root.join(".refine");
    let service = FileWorkItemService::new(&refine_dir);
    service.create_goal_summary("RACE", Some("RACE")).unwrap();
    let mutator = service.clone();
    let racing_service = service
        .clone()
        .with_after_bulk_goal_selection_hook(move || {
            mutator
                .set_goal_status_unchecked("RACE", &GoalStatus::Done)
                .unwrap();
        });

    let result = racing_service
        .bulk_update_goals(
            BulkGoalSelection {
                selected_ids: Some(vec!["RACE".to_string()]),
                ..Default::default()
            },
            BulkGoalUpdate::Status("cancelled".to_string()),
        )
    ...
    assert_eq!(result.skipped_details[0].reason, "status:done");
    assert_eq!(
        service.show_goal_summary("RACE").unwrap().goal.status,
        GoalStatus::Done
    );
