...
pub(super) fn validate_regression(regression: &RegressionCheck) -> RefineResult<()> {
...
pub(super) fn valid_regression_id(value: &str) -> bool {
...
pub(super) fn new_regression_id(title: &str, existing: &BTreeSet<String>) -> String {
...
pub(super) fn regression_spec_template(regression: &RegressionCheck, prompt: &str) -> String {
...
pub(super) fn default_viewport() -> RegressionViewport {
    RegressionViewport {
...
pub(super) fn normalize_timing(value: &str) -> RefineResult<String> {
    match value.trim() {
        ...
        POST_BUILD => Ok(POST_BUILD.to_string()),
        _ => Err(RefineError::InvalidInput(
            "timing must be one of pre_merge, post_build".to_string(),
        )),
    }
}
