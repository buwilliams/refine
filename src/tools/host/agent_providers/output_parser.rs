pub(super) fn extract_final_text(stdout: &str, output_format: &str) -> String {
...
pub(super) fn provider_error_message(stdout: &str, stderr: &str) -> Option<String> {
...
pub(super) fn extract_provider_session_id(stdout: &str) -> Option<String> {
    for line in stdout.lines() {
        let Ok(event) = serde_json::from_str::<serde_json::Value>(line) else {
            continue;
        };
...
pub(super) fn find_session_id_value(value: &serde_json::Value) -> Option<String> {
...
pub(super) fn text_from_content(content: &serde_json::Value) -> Option<String> {
    let parts = content
        .as_array()?
        .iter()
        .filter_map(|block| {
