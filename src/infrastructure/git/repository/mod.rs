...
/// Infrastructure-owned repository command mechanism used by ancestry and
/// tree operations as well as higher-level synchronization services.
#[derive(Clone, Debug)]
pub struct FileGitRepository {
    target_root: std::path::PathBuf,
    runtime_root: std::path::PathBuf,
}

impl FileGitRepository {
    pub fn new(
...
impl FileGitRepository {
    pub(crate) fn remote_exists(&self, remote: &str) -> RefineResult<bool> {
        Ok(self
            .git_stdout(&["remote"])?
            .lines()
    ...
    pub(crate) fn git_stdout(&self, args: &[&str]) -> RefineResult<String> {
        let output = self.git_checked(args)?;
        Ok(String::from_utf8_lossy(&output.stdout).trim().to_string())
    }

    pub(crate) fn git_success(&self, args: &[&str]) -> RefineResult<bool> {
        self.git(args).map(|output| output.success)
    }

    pub(crate) fn git_checked(&self, args: &[&str]) -> RefineResult<GitCommandOutput> {
    ...
    pub(crate) fn git(&self, args: &[&str]) -> RefineResult<GitCommandOutput> {
        self.git_at(&self.target_root, args)
    }

    pub(crate) fn git_at_stdout(
        &self,
        root: &std::path::Path,
        args: &[&str],
    ) -> RefineResult<String> {
...
pub(crate) fn command_failed(command: &str, output: &GitCommandOutput) -> RefineError {
