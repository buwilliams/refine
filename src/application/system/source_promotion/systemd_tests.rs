...
fn real_systemd_failed_identity_replaces_candidate_with_prior_executable() {
    ...
    assert_eq!(
        crate::infrastructure::process::supervisor::lifecycle::http_reachability_probe(port),
...
fn wait_for_reachable(port: u16, timeout: Duration) {
    let deadline = Instant::now() + timeout;
    while crate::infrastructure::process::supervisor::lifecycle::http_reachability_probe(port)
        != crate::infrastructure::process::supervisor::lifecycle::DaemonReachability::Reachable
    {
        assert!(
            Instant::now() < deadline,
