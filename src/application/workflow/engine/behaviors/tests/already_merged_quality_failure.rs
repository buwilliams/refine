...
if test "$count" -eq 1; then
  printf '%s\n' '{"ok":false,"summary":"first exact-candidate evaluation failed","results":[{"test":"Outcome works","status":"failed","evidence":"first failure","command":"false"}]}'
else
  printf '%s\n' '{"ok":true,"summary":"later evaluation passed","results":[{"test":"Outcome works","status":"passed","evidence":"later pass","command":"true"}]}'
fi
"#,
        )
        .unwrap();
    ...
    fn run_quality_once(
        &self,
    ) -> crate::application::workflow::phases::quality::QualityOperationResult {
...
pub(crate) struct SmokeAiOverride(Option<OsString>);
...
pub(crate) fn behavior_test_temp_dir(prefix: &str) -> PathBuf {
...
pub(crate) fn behavior_test_git(repo: &Path, args: &[&str]) -> String {
