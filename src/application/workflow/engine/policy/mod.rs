...
use crate::infrastructure::process::subprocess::{
    FileProcessSupervisor, ProcessOwner, ProcessPauseState,
};
...
impl ExecutionLoad {
    pub(crate) fn record(&mut self, node: &str, provider: &str, target_app: &str) {
        self.global += 1;
...
impl WorkflowEngine {
    ...
    pub(crate) fn refine_dir(&self) -> RefineResult<Option<PathBuf>> {
        self.target_root
            .as_ref()
    ...
    pub(crate) fn workflow_paused(&self) -> RefineResult<bool> {
        Ok(FileProcessSupervisor::new(&self.runtime_root)
            .pause_state()?
            .workflow_paused)
    }

    pub(crate) fn ensure_automation_running(&self) -> RefineResult<()> {
        if self.workflow_paused()? {
