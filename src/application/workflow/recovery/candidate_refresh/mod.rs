//! Refresh a proven candidate onto its current target. Failed refreshes preserve
//! evidence and enter error handling without authoring a recovery Round.
...
/// Refresh once under the repository lock. Conflicts retain the original candidate
/// and settle through Error handling; no resolver or retry is launched here.
pub(crate) fn refresh_candidate_for_target_advancement(
    ...
    with_repository_git_lock(&target_root, || {
        locked_refresh_attempt(ctx, &authority_status)
    })
}
...
fn locked_refresh_attempt(
    ...
    if let Some(reason) = provenance_error {
        // An interrupted rebase leaves a stopped pick. Abort that operation
        // while retaining the exact original candidate and failure evidence.
...
/// Persist a completed clean rebase as the replacement candidate.
...
fn settle_refresh_failure(
    ...
    let _ = recovery;
    Ok(CandidateRefreshOutcome::Stopped {
