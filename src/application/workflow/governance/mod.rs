...
use crate::application::agent_io::structured_output::{
    Contract, DecodeOptions, Selection, select_value,
};
...
pub(crate) fn post_implementation_governance_prompt(
    governance: &Value,
    rules: &[Value],
    guidance: &[Value],
    worktree_path: &str,
...
pub(crate) fn plan_governance_precheck_prompt(
    governance: &Value,
    rules: &[Value],
    guidance: &[Value],
    provider_cwd: &Path,
...
pub(crate) struct GovernanceVerdictExample {
    status: String,
    message: String,
...
pub(crate) const GOVERNANCE_VERDICT_UNPARSABLE: &str = "Governance verdict could not be parsed: the review did not return the required JSON verdict object.";

pub(crate) fn parse_governance_provider_output(
    output: &str,
    rules_checked: usize,
...
mod contract_tests {
    ...
    fn verdict_contract_example_roundtrips_and_parses_as_a_verdict() {
        crate::application::agent_io::structured_output::assert_contract_roundtrip::<
            GovernanceVerdictExample,
        >();
        let evaluation =
