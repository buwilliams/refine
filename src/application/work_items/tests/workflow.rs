...
#[test]
fn integration_failure_retains_source_evidence_without_a_successor() {
    ...
    let recovered = service
        .settle_integration_failure_summary(
            "GOAL1",
            authority,
            &GoalStatus::Governance,
            "default",
    ...
    assert_eq!(recovered.goal.status, GoalStatus::Failed);
    assert_eq!(recovered.goal.round_count, 1);
    let detail = service.show_goal_detail("GOAL1").unwrap();
    let rounds = detail["rounds"].as_array().unwrap();
    assert_eq!(rounds.len(), 1, "{detail:#}");
    assert_eq!(
        rounds[0]["workflow_attempt_authority"]["workflow_revision"],
        authority.workflow_revision
    );
    assert_eq!(rounds[0]["failure_category"], "integration");
    assert_eq!(
        rounds[0]["workflow_recovery"]["retained_evidence"]["kept"],
        true
    );
    assert!(rounds[0]["workflow_recovery"]["successor_round"].is_null());
    assert_eq!(rounds[0]["workflow_recovery"]["automatic_recovery"], false);
    fs::remove_dir_all(temp_root).unwrap();
}
