...
/// from `src/surfaces/refine-hub/docs/intent/02-model/04-fleet.md` (a test pins the quote to the
...
mod installed_agent;
pub use installed_agent::InstalledAgentResolver;
#[cfg(test)]
use installed_agent::resolution_prompt;
...
mod tests {
    ...
    fn ownership_doctrine_is_quoted_from_the_fleet_intent() {
        let intent =
            include_str!("../../../../src/surfaces/refine-hub/docs/intent/02-model/04-fleet.md");
        let normalize = |text: &str| text.split_whitespace().collect::<Vec<_>>().join(" ");
        assert!(
            normalize(intent).contains(&normalize(OWNERSHIP_DOCTRINE)),
            "the doctrine quote drifted from src/surfaces/refine-hub/docs/intent/02-model/04-fleet.md"
        );
    }
