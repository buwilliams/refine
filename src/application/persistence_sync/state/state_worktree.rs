...
impl FileGitSyncService {
    pub(crate) fn recover_interrupted_state_worktree(
        &self,
    ...
    pub(crate) fn restore_managed_state_worktree(&self) -> RefineResult<()> {
    ...
    pub(crate) fn fetch_remote(&self, remote: &str) -> RefineResult<()> {
        self.git_checked(&["fetch", "--prune", remote]).map(|_| ())
    }

    pub(crate) fn remote_state_exists(&self, remote: &str) -> RefineResult<bool> {
        Ok(self
            .git(&[
                "ls-remote",
                "--exit-code",
    ...
    pub(crate) fn remote_state_tracking_exists(&self, remote: &str) -> RefineResult<bool> {
        self.git_success(&[
            "show-ref",
            "--verify",
            "--quiet",
    ...
    pub(crate) fn fetch_state_branch(&self, remote: &str) -> RefineResult<()> {
    ...
    pub(crate) fn ensure_state_worktree(
        &self,
        remote: &str,
        remote_exists: bool,
    ...
    pub(crate) fn ensure_local_state_excluded(&self) -> RefineResult<()> {
    ...
    pub(crate) fn configured_remote(&self, refine_dir: &std::path::Path) -> RefineResult<String> {
        let settings =
