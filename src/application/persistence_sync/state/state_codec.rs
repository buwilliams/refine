...
#[derive(Debug)]
pub(crate) struct StateWorktreeSetup {
    pub(crate) path: PathBuf,
    pub(crate) pulled: bool,
    pub(crate) created: bool,
}

pub(crate) type DurableStateMap = BTreeMap<PathBuf, u64>;

pub(crate) fn state_content_fingerprint(bytes: &[u8]) -> u64 {
    let mut hasher = DefaultHasher::new();
    bytes.hash(&mut hasher);
    hasher.finish()
}

pub(crate) fn is_bootstrap_state_path(path: &std::path::Path) -> bool {
    matches!(
...
pub(crate) fn durable_state_map(root: &std::path::Path) -> RefineResult<DurableStateMap> {
    if !root.exists() {
...
pub(crate) fn state_change_status(
...
pub(crate) fn record_synchronized_goal(
...
pub(crate) fn forget_synchronized_goal(live_root: &std::path::Path, relative: &std::path::Path) {
    if !is_goal_record(relative) {
        return;
    }
...
pub(crate) fn state_record_key(path: &str, prefix: &str) -> Option<String> {
