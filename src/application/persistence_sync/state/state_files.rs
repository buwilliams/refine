...
pub(crate) fn collect_durable_state_files(
    root: &std::path::Path,
...
pub(crate) fn remove_excluded_state_files(root: &std::path::Path) -> RefineResult<Vec<PathBuf>> {
    if !root.exists() {
        return Ok(Vec::new());
    }
...
pub(crate) fn remove_excluded_state_files_from(
    root: &std::path::Path,
    current: &std::path::Path,
...
pub(crate) fn is_transient_refine_path(path: &std::path::Path) -> bool {
...
pub(crate) fn is_runtime_only_refine_path(path: &std::path::Path) -> bool {
    if matches!(
        path.components()
            .next()
    ...
    if path == std::path::Path::new(crate::application::fleet::nodes::ACTIVE_NODE_FILE) {
        return true;
    }
...
pub(crate) fn is_excluded_from_durable_state(path: &std::path::Path) -> bool {
    is_runtime_only_refine_path(path) || is_transient_refine_path(path)
}

pub(crate) fn append_output_detail(details: &mut Vec<String>, output: &GitCommandOutput) {
...
pub(crate) fn nonempty_detail(details: Vec<String>) -> Option<String> {
    let detail = details.join("\n");
    (!detail.is_empty()).then_some(detail)
}

pub(crate) fn push_rejected_by_race(output: &GitCommandOutput) -> bool {
    let text = format!(
        "{}\n{}",
