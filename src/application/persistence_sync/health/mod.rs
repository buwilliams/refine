...
// `StateSyncRecoveryKind` is retired: its only variant, `missing_baseline`,
// lost its producer when the merge-base pipeline deleted the baseline system.
// Durable records written by older binaries may still carry a
// `recovery_kind` member; deserialization ignores unknown members, so those
// records read back cleanly and the retired kind simply disappears.
...
impl FileStateSyncHealthService {
    ...
    /// must still carry the observed revision.
    pub fn record_recovery_success(
        ...
        let activity = self.update_if(target_root, node_id, |record| {
            (record.revision == expected_revision).then(|| settle_success_record(record))
        })?;
        Ok(match activity {
