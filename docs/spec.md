...
- **Parallel-run cap** — at most N agent CLI subprocesses may run at once across all Gaps (default `5`, configurable from the System page). There is no persistent worker pool; subprocesses are spawned on demand when there is work to do, and the process exits when its round finishes.
...
|-----------------------|--------------------------------------------|-------|
| Parallel-run cap      | `5`                                        | Max agent subprocesses running concurrently. |
