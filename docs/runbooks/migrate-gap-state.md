...
In v4, every `.refine` path below refers to the tree in the isolated
`<app>/.git/refine-state-worktree/` checkout of `refine/state`. It must not be
created as `<app>/.refine` in the primary target-app worktree.
