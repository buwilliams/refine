...
- `project state-recovery run` is the consolidated one-shot recovery: it
  ...
  verifies with a fresh synchronization. Authority defaults to `remote` because
  live authority republishes the node's divergent state. Bounded races against
  a moving remote head or a concurrent live write are retried inside the
  command; callers never wrap it in retry loops, and every non-race failure
  surfaces immediately as itself.
