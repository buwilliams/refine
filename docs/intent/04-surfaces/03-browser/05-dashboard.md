...
- Git-busy recovery rejection retains the preview but invalidates confirmation before retry. Stale-preview rejection clears the preview and choice until a fresh preview is reviewed. Successful recovery presents the resulting heads, authority, settled paths, and any retained refs while authoritative state-sync health refreshes and shows the failure cleared;
