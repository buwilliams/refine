...
The shared prompt serializer renders pinned context as readable Markdown. Planning phases must leave Git unchanged. Completed proposal, criticism, final-plan, checklist, verification, governance, and implementation artifacts are synchronized semantic evidence; process, operation, and session identifiers are local and are not copied into those artifacts. Completion signals use one typed contract across phases, including zero-based integer indexes for applied Guidance. The agent writes and parse-checks a temporary completion signal before atomically replacing the active signal. Refine still tolerates a briefly partial legacy direct write; after a stable JSON or typed-schema rejection — including a planning-phase completion that omits the required `planning_result` object — it preserves the bad payload and diagnostic under attempt-numbered, process-scoped runtime paths, asks the same live agent to rewrite it, and accepts the first valid replacement. Three rejected replacement payloads, or an agent exit before correction, end recovery with the final diagnostic and preserved evidence. The configured agent hard cap continues to bound the whole live session.
