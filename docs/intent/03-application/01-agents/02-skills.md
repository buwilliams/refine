...
The Skill modal leads with rendered Markdown instructions and an edit icon for Markdown authoring. Its title identifies the Skill. A single Skill settings section below the instructions reveals the name, Trigger, Scope, Status, parameters, and workflow options; nested collapsible editors are unnecessary. Trigger, Scope, and Status share one row and stack on narrow screens. A Skill and its trigger save atomically under an observed revision; concurrent edits fail without erasing the current configuration. Deleting a Skill removes its trigger. Editing instructions never rewrites completed invocation or Round evidence. Clone Skill creates an independent copy with a new identity.
