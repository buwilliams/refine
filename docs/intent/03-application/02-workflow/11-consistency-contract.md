...
Target advancement is distinct from provenance failure. Refresh requires an exactly resolving base and candidate, base ancestry of both candidate and target, a clean candidate branch still naming the candidate, and a linear unambiguous delta. A rebase conflict first receives bounded in-place agent resolution over the preserved rebase — the conflicted workspace is the durable state, so a crash is answered by the next attempt's provenance checks — and a resolved replacement re-passes the same linearity and ancestry gate before persisting with its resolution note. Provenance ambiguity, resolution failure or exhaustion, or the resolution opt-out aborts and cleans the attempted refresh without moving or deleting the original candidate, then one Goal-lock and attempt-fenced mutation retains original branch, worktree, handoff, gates, target snapshot, and conflict details — with the resolution attempts and, on exhaustion, a domain-terms question — while appending a recovery Round. Its `automatic_retry` record uses kind `integration`, the source Round, and the next monotonic attempt shared with Quality and Governance; exhaustion records the retained evidence and terminal outcome without appending an unbounded successor. A conflicted integration merge follows the same Goal-lock and attempt-fenced mutation: retained conflict evidence, `automatic_retry.kind = integration`, the source Round, and the next monotonic attempt in the shared budget, with exhaustion recorded without an unbounded successor.
