...
The governing requirement is stable progress under current intent. Every accepted action must leave the daemon able to restore usable execution after interruption. Surfaces record requests through shared Application capabilities; the daemon completes cleanup and processing independently of the request thread. This uses the existing workflow owners without a second execution-ownership state machine.
...
Workflow outcomes are controlled through a shared Application capability exposed by API, CLI, MCP, and Web. Humans, agents, and Skills consume that capability. Requests carry expected revision, request identity, reason, and optional context. A later accepted decision supersedes earlier work; stale callbacks cannot overwrite it. Selecting a step retains the authored Round, including Plan. Selecting the current step is a no-op; Retry and Integrate remain distinct operations. Overrides can select a step without its generated prerequisites or successful checks. The decision is recorded first, superseding pending outcomes, transitions, and old results; daemon maintenance then stops obsolete Goal processes, including the submitting Skill. Preparation restores usable inputs or records necessary recovery through the existing workflow. Concurrent-edit checks protect the intended decision. Forced Done is status-only; forced integration is a separate operation against a pinned candidate.
...
Workers persist artifacts and reread current Goal decisions at transitions and consequential boundaries. Continuous reconciliation checks every executable step for a live execution owner or an admission constraint. Eligible work can resume after prior execution is proven stopped, including when a scheduler still emits healthy ticks. Restart uses the same assessment and retains completed verdicts. Shared preparation reuses coherent base/branch/candidate identities, reconstructs missing derived work, or records a return to Plan on a fresh execution branch under the existing authored Round. Old generated work is reproducible; it must not permanently block that Round. This recovery does not invent a pass, silently retry a failed Skill verdict, or reopen a parked Goal. Provider, integration, and report-contract errors retain their existing Error-handling behavior.
