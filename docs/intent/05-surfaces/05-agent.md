...
- **Agents Act, Application Owns Semantics**: agents execute workflow steps and use host tools; they should not redefine workflow semantics on their own.
- **Future Dominance**: over time, agent-native interaction may become the most important surface.

## Purpose

The agent Surface exists because Refine is designed for AI-assisted and eventually AI-led software work. Agents need a reliable interface for understanding the Application, inspecting work, invoking its behavior, producing changes, and handing work back for Quality, review, merge, or further automation.
...
Agents should use the same Model and Application as people through the CLI and other Surfaces: Goal, Feature, workflow state, guidance, Governance, logs, changes, and processes. Their outputs should become durable work evidence, not disappear into provider transcripts.
...
As AI systems become stronger, they may compress Refine into workflow, persistence, and orchestration. That is acceptable if they preserve the intent: durable state, inspectable work, local ownership, shared Application behavior, and enough guidance for future agents to understand why the system exists.
