...
- `refine config` reads ordinary Settings and can select the Events or Skills domain. The `events` and `skills` command groups expose revision-fenced definition editing. Events additionally exposes catalog, bindings, manual triggers, invocation status, history, and cancellation. Trigger input supports typed parameters, defaults, Goal context, and interactive prompts for missing required values. The retired Governance, Quality, and Guidance configuration menus are removed.
