...
The tab order is Processes, Application, Reporters, Skills, Target App, Runtime, and Refine (dev).
...
- the Skill editor has one Trigger field, instructions, scope, enabled state, and optional typed parameters. Scope and enabled state use shared segmented controls. Workflow options and parameter context are collapsed details. There is no result-role or project-assignment override control. Clone Skill creates an independent copy for another trigger;
