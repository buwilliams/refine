...
Implementation planning persists typed proposal, criticism, revision, and implementation evidence with Goal/Round, context digest, branch, target branch, and base commit. Updates compare against the complete prior planning value. Invalid structured responses and diagnostics are appended before each of at most two repair invocations, which resume from the last valid phase artifact. They do not persist the identity of the process that produced the accepted plan. A Quality- or Governance-finding recovery Round persists a synthesized proposal and final plan under the same binding and comparison rules instead of invoking planning agents, and a pinned provider-native session identifier is continuity metadata only — losing it degrades to a fresh launch, never to a failed Round.
