...
- Exact Goal-round fields and the Goal log must be durable before the operation reports success or failure; persistence failures leave the operation nonterminal for restart recovery.
...
The authoritative Quality timing controls ordering: `pre_merge` runs QA before the target-app build, while `post_build` runs QA after a successful build. Provider, parsing, cancellation, and restart-interruption outcomes remain visible instead of being converted to generic success or hidden retries; provider and test launches recheck operation state and share the process-registration/cancellation barrier.
