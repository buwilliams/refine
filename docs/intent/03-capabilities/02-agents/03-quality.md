...
- evaluation should persist an exact identity commitment and revalidate it before and after supervised checks: `isolated_candidate` requires the current Goal Round, candidate branch, registered worktree path, clean checkout, and candidate HEAD, while legacy reconciliation may validate the exact source candidate and integrated target branch/commit without requiring the candidate worktree to remain present;
- implementation worktree creation should register a node-local candidate handoff under the repository maintenance lock. That cleanup-visible owner remains active through implementation and transfers without a cleanup gap to Quality, Governance, and publication;
