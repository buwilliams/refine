...
- evaluation should persist a scope-specific identity commitment and revalidate it before and after supervised checks: `isolated_candidate` requires the exact current Goal Round, candidate branch, registered worktree path, clean checkout, and candidate HEAD, while `integrated_target` and `integrated_target_reconciliation` require the exact source candidate and Ready Merge target branch/commit in the clean target checkout without requiring the candidate worktree to remain present;
- implementation worktree creation should register a node-local candidate handoff under the repository maintenance lock. That cleanup-visible owner should remain active through Goal Agent settlement, candidate commit, Governance, and publication, then transfer without a cleanup gap to the isolated-candidate Quality operation after the durable QA transition. Post-build work should retain the handoff until exact integration evidence makes the candidate checkout unnecessary;
