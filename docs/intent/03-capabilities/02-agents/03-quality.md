...
- evaluation should persist an exact identity commitment and revalidate it before and after supervised checks: `isolated_candidate` requires the current Goal Round, candidate branch, registered worktree path, clean checkout, and candidate HEAD. A sibling Goal advancing a shared target does not change that isolated evaluation identity. Already-merged reconciliation never substitutes a new merged-target Quality run; it may advance only from durable passed isolated Quality and Governance evidence bound to the current Round and exact candidate;
