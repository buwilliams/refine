...
- **Node**: the local or distributed actor responsible for owning and advancing work.
...
- Node ownership is represented on work items so distributed or multi-instance operation can be reasoned about explicitly.
- Projections exist so the system can stay fast without replacing flat files as the source of truth.

Future model changes should preserve these properties: human readability, agent readability, stable workflow meaning, and clear ownership.
