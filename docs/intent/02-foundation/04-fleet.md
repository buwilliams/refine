...
- State syncs symmetrically on one dedicated `refine/state` branch. Each node uses an isolated state worktree, applies its local state delta, detects same-record races, creates one summarized batch commit, and pushes without checking out, staging, pulling, or pushing the application branch. Durable mutations request synchronization through a short trailing debounce and fetch only the state branch. Separately, the existing configurable project update pulse fetches all remote branches every five minutes by default so Refine observes human application commits without moving the checked-out branch. Manual Sync now bypasses both cadences.
