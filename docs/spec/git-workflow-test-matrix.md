...
| Automatic reconciliation with no Git repository or upstream | daemon reconciler through `FileGitSyncService` | `web_server_project_sync_reports_no_git_repo_and_missing_upstream`, reconciler cadence unit coverage |
| Commit and push durable `.refine` state, then pull it on another node | daemon reconciler through `FileGitSyncService` | `sync_commits_pushes_and_pulls_refine_state`, `multi_instance_sync.rs` |
...
| Defer and retry safely while target-app files are active | daemon reconciler through `FileGitSyncService::try_sync` | `sync_does_not_touch_uncommitted_target_app_changes`, `web_server_project_sync_skips_pull_for_dirty_user_worktree` |
