...
| Revert commit cleanly | `FileGitWorktreeService::revert_commit` | `file_git_worktree_service_lists_status_and_reverts_commits`, `web_server_lists_git_changes_and_reverts_commits`, `confirms Changes undo, reverts git, and cancels the linked Gap` |
...
| Project sync with no Git repository | `sync_attached_project_git_state` via `/api/project/sync` | `web_server_project_sync_reports_no_git_repo_and_missing_upstream`, `fixture.assert_success("project sync", ...)` in `tests/cli_surface.rs` |
| Project sync with no upstream | `sync_attached_project_git_state` via `/api/project/sync` | `web_server_project_sync_reports_no_git_repo_and_missing_upstream` |
| Project sync clean fast-forward pull | `sync_attached_project_git_state` via `/api/project/sync` | `web_server_project_sync_pulls_fast_forward_and_allows_refine_runtime_noise`, `multi_instance_sync.rs` |
| Project sync dirty-worktree skip | `sync_attached_project_git_state` via `/api/project/sync` | `web_server_project_sync_skips_pull_for_dirty_user_worktree` |
| Project sync `.refine/runtime` exception | `project_sync_status_line_blocks_pull` | `web_server_project_sync_pulls_fast_forward_and_allows_refine_runtime_noise` |
| Project sync pull failure | `sync_attached_project_git_state` via `/api/project/sync` | `web_server_project_sync_reports_pull_failure_for_diverged_branch` |
...
| Changes Undo API and UI | `handle_changes_undo`, `changes.js` | `web_server_lists_git_changes_and_reverts_commits`, `confirms Changes undo, reverts git, and cancels the linked Gap` |
