...
| Git Visualization by day/week/month/year | UI | Covered post-baseline | Changes now has URL-backed Day/Week/Month/Year visualization controls. Playwright seeds dated git commits in the disposable target app, syncs the projection, and asserts filtered day/week/month/year buckets plus linked-Gap counts. |
| Branch info and unresolved empty state | UI/Route | Covered post-baseline | `/api/changes` now returns the inspected source branch or `null` instead of masking no-branch state as `main`; Playwright asserts branch info on seeded Changes rows, resolved-branch empty text, and detached-HEAD unresolved branch guidance using the disposable git app. |
...
| Application node table activate/rename/archive/create | UI/CLI | Covered post-baseline | Playwright creates an Application-tab node, activates it, renames it, restores the original active node, archives the created node through the danger confirmation, and verifies each `/api/nodes` payload plus row state. CLI node create/list/show/rename/settings/activate/archive/transfer coverage exists in `tests/cli_surface.rs`. |
...
| System operations log filters/count/limit/empty/no-match | UI/Journey | Covered post-baseline | Public file API error records browser-visible system operations; Playwright covers All/Error/Queued filters, count, empty/no-match states, persisted filter state across reload via toolbar storage, and the 250-entry visible/total limit. |
...
| Gap rounds collapse/edit latest/follow-up/recovery | UI/CLI | Covered post-baseline | Gap detail Playwright coverage edits the latest backlog/todo round, submits a follow-up round from review, submits a recovery round from failed, and asserts rendered round details plus public Gap `round_count`. Native CLI coverage appends and edits the latest round through daemon-backed `gap round`. |
| Gap notes add/edit/delete | UI/CLI | Covered post-baseline | Gap detail Playwright coverage adds, expands, edits, and deletes a note through the browser, including the delete confirmation and empty state. Rust route coverage supports the UI's notes replacement payload, and native CLI coverage exercises `gap note`, `gap note-edit`, and `gap note-delete` through daemon-backed `gap show` assertions. |
...
| `gap note/round` | Notes/rounds | Covered post-baseline | Native CLI suite covers note add/edit/delete plus round append and edit-latest through daemon-backed public commands. |
