...
function fixture(url,request) {
  ...
  if(url==='/api/processes')return {runner_reachable:true,workflow_health:{healthy:true,state:'running'},paused:false,target_app:{state:"running",has_start_action:true,has_stop_action:true,has_build_action:true,has_status_checks:true},processes:[
    {id:'daemon',kind:'daemon',status:'running',pid:4100,label:'Refine daemon'},
    {id:'agent-1',kind:'agent',provider:'codex',status:'running',pid:4201,label:'Search result previews',goal_id:goals[2].id},
    {id:'agent-2',kind:'agent',provider:'claude',status:'running',pid:4202,label:'Navigation verification',goal_id:goals[3].id},
...
(async()=>{
  ...
  try {
    ...
    await page.addInitScript(() => localStorage.setItem("rail-skills-section", "false"));
