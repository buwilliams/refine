...
Quality does not silently retry completed failed verdicts. Provider, parsing, test-harness, and integration errors retain their Error/default-Failed behavior. Before evaluation, the [shared preparation capability](11-consistency-contract.md) may reconstruct usable inputs or record recovery to Plan on a fresh execution branch under the same authored Round. This does not fabricate a pass or accept stale candidate evidence. Standalone worktree handoff enters Quality directly.
