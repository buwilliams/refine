...
Skill instructions and current user authorization determine permitted edits and supported workflow changes, including during Plan and Governance. Exact-candidate verification remains observational; a verdict for one candidate cannot authorize a different candidate. Refine does not execute commands supplied in reports. Workflow agents use the existing Goal worktree. Checkout serialization, runtime capacity, pause controls, completion and idle timeouts, process cancellation, and current Goal authority apply to Event agents.
