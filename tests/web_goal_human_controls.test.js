...
test("Goal actions follow current status, retain every step, and recover from rejected moves", async () => {
  ...
  try {
    ...
    assert.equal(await latestRound.getByRole("tab", { name: "Plan", exact: true }).getAttribute("aria-selected"), "true");
    await page.keyboard.press("ArrowRight");
    assert.match(await latestRound.getByRole("tabpanel").textContent(), /No Implement artifacts/);
    await latestRound.getByTestId("goal-round-summary").click();
    await page.evaluate(() => drawGoalDetail(goal));
    assert.equal(await latestRound.getAttribute("open"), null);
    await latestRound.getByTestId("goal-round-summary").click();
    assert.equal(await latestRound.getByRole("tab", { name: "Implement", exact: true }).getAttribute("aria-selected"), "true");
