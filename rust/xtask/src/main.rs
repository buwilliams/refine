...
fn check_static_assets() -> Result<(), String> {
    ...
    let python_static = repo_root.join("python/refine_ui/static");
    let rust_static = repo_root.join("rust/src/surfaces/web/static");
    let python_assets = collect_files(&python_static)?;
    let rust_assets = collect_files(&rust_static)?;
    if python_assets != rust_assets {
        return Err(format!(
            "Rust static assets differ from Python baseline\npython={python_assets:#?}\nrust={rust_assets:#?}"
        ));
    }
    println!(
        "{}",
        serde_json::to_string_pretty(&json!({
            "static_assets_match": true,
            "files": rust_assets.len()
        }))
