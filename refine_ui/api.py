...
def _select_bulk_update_candidates(
    ...
    *,
    ...
    if status:
        where.append("status = ?")
        args.append(status)
    if q and not needs_round_search:
        where.append("(name LIKE ? OR id LIKE ?)")
        like = f"%{q}%"
        args.extend([like, like])
    if reporter:
    ...
    try:
        ...
        conn.close()
    if needs_round_search:
        rows = [r for r in rows if _bulk_gap_matches_query(r, q)]
...
def _bulk_gap_matches_query(row: dict[str, Any], q: str) -> bool:
    needle = q.lower()
    if needle in str(row.get("name") or "").lower():
        return True
    if needle in str(row.get("id") or "").lower():
        return True
    gap = shared_gaps.read_gap_json(str(row.get("id") or ""))
    if not gap:
        return False
    for round_obj in gap.get("rounds", []):
        blob = " ".join([
