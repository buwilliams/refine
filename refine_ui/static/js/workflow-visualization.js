      ...
      ${statuses.map((s) => {
        ...
        const body = `
          <div class="workflow-status-head">
            ...
            <div class="workflow-status-label">${workflowStatusLabel(s)}</div>
          </div>
          <div class="workflow-status-count">${fmtCount(count)}</div>`;
        const attrs = `class="card workflow-status-card ${s}${agentManaged ? " workflow-status-card-agent" : ""}" title="${count} ${workflowStatusLabel(s)} gap${count === 1 ? "" : "s"}${agentManaged ? " - agent-managed automation" : ""}"`;
